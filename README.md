...
본 함수가, 일차함수이고 계수 및 상수값도 작아서 **증가폭이 작게 느껴질 수 있지만**, 전 세대보다는 평균 적합도가 훨씬 크게 증가한 것을 알 수 있다. 이제 이 과정을 반복하는데, 최종적으로 정확한 해는 구할 수 없으므로, 가장 적합도가 높은 후보해를 리턴할 때 알고리즘을 종료한다.
